...
class Subscriber {
  ...
  static async findSubscriberById(id) {
    const query = {
  ...
  static async addSubscriber(name, lastName, email, phone) {
    const query = {
...
newsletter.get("/", async (c) => {
  const newsletter2 = await Newsletter.findAllNewsletters();
  return c.json({
    data: newsletter2
  }, 200);
});
